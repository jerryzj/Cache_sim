...
class ArgumentParser {
  ...
  public:
    ...
    void print_help() {
        ...
        for (auto &a : _arguments) {
            ...
            if (i != _pairs.end())
                name.append(", " + i->second);
            std::cout << "    " << std::setw(23) << std::left << name
                      << std::setw(23)
                      << a._desc + (a._required ? " (Required)" : "")
                      << std::endl;
        }
    }
    ...
    bool exists(const std::string &name) {
        ...
        if (_pairs.find(t) != _pairs.end())
            t = _pairs[t];
        return _variables.find(t) != _variables.end();
    }

    template <typename T> std::vector<T> getv(const std::string &name) {
  ...
  private:
    ...
    inline void _add_variable(std::string name,
        ...
        name = _delimit(name);
        if (_pairs.find(name) != _pairs.end())
            name = _pairs[name];
...
ArgumentParser::getv<std::string>(const std::string &name) {
    std::string t = _delimit(name);
    if (_pairs.find(t) != _pairs.end())
        t = _pairs[t];
    auto v = _variables.find(t);
